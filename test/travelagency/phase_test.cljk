(ns travelagency.phase-test
  "Unit tests of `travelagency.phase` rollout logic."
...
(deftest phase-1-booking-record-only
  (testing "phase 1 allows only booking-record logging, requires approval"
    (let [{:keys [disposition reason]} (phase/gate 1 {:op :log-booking-record} :commit)]
      (is (= :escalate disposition))
      (is (= :phase-approval reason)))
    (let [{:keys [disposition]} (phase/gate 1 {:op :schedule-booking-operation} :commit)]
      (is (= :hold disposition)))))

(deftest phase-2-adds-coordination-ops
  (testing "phase 2 allows coordination ops, still requires approval"
    (doseq [op [:log-booking-record :schedule-booking-operation :coordinate-vendor-settlement]]
...
(deftest phase-3-auto-commits-clean-ops
  (testing "phase 3 auto-commits clean, high-conf non-safety ops"
    (let [{:keys [disposition]} (phase/gate 3 {:op :log-booking-record} :commit)]
      (is (= :commit disposition)))
    (let [{:keys [disposition]} (phase/gate 3 {:op :schedule-booking-operation} :commit)]
      (is (= :commit disposition)))
...
(deftest hard-hold-always-wins
  (testing "a governor HARD hold stays HOLD regardless of phase"
    (doseq [ph [0 1 2 3]]
      (let [{:keys [disposition]} (phase/gate ph {:op :log-booking-record} :hold)]
        (is (= :hold disposition)
